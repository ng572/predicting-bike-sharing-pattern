...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        """
        hidden_inputs = np.zeros((self.hidden_nodes,))
        for index in np.nonzero(X)[0]:
            hidden_inputs += X[index] * self.weights_input_to_hidden[index]
        """
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        """
        for index_r in np.nonzero(X)[0]:
            for index_c in range(len(hidden_error_term)):
                delta_weights_i_h[index_r][index_c] += X[index_r] * hidden_error_term[index_c]
        """
        delta_weights_i_h += X[:, None] * hidden_error_term # shape np
...
##########################################################
iterations = 3000
learning_rate = 1.5 # because we divide by n_records
output_nodes = 1
hidden_nodes = 7
    # need to think of the meta-variables here
        # comfort
            #weathersit
            #temp
            #atemp # dropped
